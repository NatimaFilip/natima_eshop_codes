const translationsStrings = {
	...
	natiosSupportTotalAmount: {
		cs: "Společně jsme již dokázali přispět přes 915&nbsp;000&nbsp;Kč. Děkujeme!",
		sk: "Spoločne sme už dokázali prispieť cez 915&nbsp;000&nbsp;Kč. Ďakujeme!",
		pl: "Wspólnie udało nam się już przekazać ponad 915&nbsp;000&nbsp;CZK. Dziękujemy!",
	},
	...
	raventicNoResults: {
		...
		sk: "Nič sme nenašli, skúste to znova",
		pl: "Nic nie znaleźliśmy, spróbuj ponownie",
	},
	raventicResultsTitle: {
		cs: "Hledáme",
		sk: "Hľadáme",
		pl: "Szukamy",
	},
	raventicNoQueryTitle: {
		cs: "Produkty",
		sk: "Produkty",
		pl: "Produkty",
	},
	raventicCategoriesTitle: {
		cs: "Kategorie",
		sk: "Kategórie",
		pl: "Kategorie",
	},
	raventicManufacturersTitle: {
		cs: "Výrobce",
		sk: "Výrobca",
		pl: "Producent",
	},
	...
	raventicFiltersTitle: {
		cs: "Filtry",
		sk: "Filtre",
		pl: "Filtry",
	},
	raventicFilterButton: {
		cs: "Zobrazit filtry",
		sk: "Zobraziť filtre",
		pl: "Pokaż filtry",
	},
	raventicClearAllFilters: {
		cs: "Zrušit všechny filtry",
		sk: "Zrušiť všetky filtre",
		pl: "Usuń wszystkie filtry",
	},
	raventicParamDescription: {
		cs: "Popis",
		sk: "Popis",
		pl: "Opis",
	},
	...
	raventicParamReviewCount: {
		cs: "Počet recenzí",
		sk: "Počet recenzií",
		pl: "Liczba recenzji",
	},
	raventicParamAltImage: {
		cs: "Alternativní obrázek",
		sk: "Alternatívny obrázok",
		pl: "Alternatywny obrazek",
	},
	raventicParamStock: {
		cs: "Skladem",
		sk: "Skladom",
	...
	raventicWelcome: {
		cs: "Najděte přesně to, co hledáte!",
		sk: "Nájdite presne to, čo hľadáte!",
		pl: "Znajdź dokładnie to, czego szukasz!",
	},
	raventicRecommendedProducts: {
		cs: "Top produkty",
		sk: "Top produkty",
		pl: "Top produkty",
	},
	raventicFoundProducts: {
		cs: "Top produkt",
		sk: "Top produkt",
		pl: "Top produkt",
	},
	raventicSearchingFor: {
		cs: "Hledáme frázi",
		sk: "Hľadáme frázu",
		pl: "Szukamy frazy",
	},
	raventicWouldYouSearchFor: {
		cs: "chcete raději hledat",
		sk: "chcete radšej hľadať",
		pl: "czy chcesz raczej wyszukać",
	},
	raventicShowAll: {
	...
	raventicCloseLabel: {
		cs: "Zavřít vyhledávání",
		sk: "Zavrieť vyhľadávanie",
...
window.NAT.lang = window.NAT.lang || document.documentElement.lang || "cs";

window.NAT.translations = function (key) {
	const entry = translationsStrings[key];
	if (!entry) {
		console.warn(`[NAT.translations] missing translation key: "${key}"`);
		return key;
	}
	return entry[window.NAT.lang] ?? entry.cs ?? key;
};
...
window.NAT.raventic = window.NAT.raventic || {
	...
	sk: {
		...
		currency: "EUR",
		priceDecimals: 2,
		submitUrl: "https://www.natima.sk/vyhladavanie/",
		resultsUrlMatch: "www.natima.sk/vyhladavanie",
	},
	pl: {
		apiKey: "euc8c5f31012cea7c050d4b3c2d0467",
		bestsellersApiKey: "euc8c5f31012cea7c050d4b3c2d0467",
		...
		currency: "PLN",
		priceDecimals: 2,
		submitUrl: "https://www.natima.pl/wyszukiwanie/",
		resultsUrlMatch: "www.natima.pl/wyszukiwanie",
	},
};
...
// window.RaventicSearchDropdownConfig and runs Raventic's loader snippet, which
// injects the v4 dropdown SDK. The minified loader IIFE is vendor code, left untouched.

raventicSetupDropdown();
function raventicSetupDropdown() {
	...
	window.RaventicSearchDropdownConfig = {
		...
		config: {
			...
			bestsellersApiKey: window.NAT.raventic[window.NAT.lang].bestsellersApiKey,
			currency: window.NAT.raventic[window.NAT.lang].currency,
			priceDecimals: window.NAT.raventic[window.NAT.lang].priceDecimals,

			welcomeMessage: window.NAT.translations("raventicWelcome"),
			recommendedProductsTitle: window.NAT.translations("raventicRecommendedProducts"),
			foundProductsTitle: window.NAT.translations("raventicFoundProducts"),
			foundProductsTitleShort: window.NAT.translations("raventicFoundProducts"),
			noResultsFoundMessage: window.NAT.translations("raventicNoResults"),
			searchingForMessage: window.NAT.translations("raventicSearchingFor"),
			wouldYouLikeToSearchForMessage: window.NAT.translations("raventicWouldYouSearchFor"),
			...
			defaultButtonLabel: window.NAT.translations("raventicAddToCart"),
			cartConfig: {
				type: "NoActionCart",
			},
			...
			individualParameters: [
				{
					...
					title: window.NAT.translations("raventicParamDescription"),
				},
				/* 				{
			...
			showAllMessage: window.NAT.translations("raventicShowAll"),
	...
	// Vendor Raventic v4 loader — reads window.RaventicSearchDropdownConfig, wires the
	// click-capture machinery, and injects .../semsearch/v4/dropdown.min.js. Left
	// exactly as provided by Raventic.
	!(function () {
		const e = window.RaventicSearchDropdownConfig;
		if (!e)
			return void console.error(
				"RaventicSearchDropdownConfig is not defined. Please set window.RaventicSearchDropdownConfig before loading this script.",
			);
		const {
			elementSelectors: t,
			config: n,
		...
		if (l) {
			const e = "search_dropdown",
				t = "RaventicSearchIntegrationTest";
			...
			try {
				...
				a = document.cookie
					.split("; ")
					.find((e) => e.startsWith(n + "="))
					?.split("=")[1];
			}
			variant = a === o || a === r ? a : Math.random() < 0.5 ? o : r;
			try {
				window.localStorage.setItem(n, variant);
			} catch {}
			if (
				((document.cookie = `${n}=${variant}; domain=${location.hostname}; path=/; max-age=${365 * 86400}; sameSite=Lax`),
				t &&
					window.dataLayer.push({ event: t, raventic: { search: {}, raventic_variant: variant, raventic_widget: e } }),
				variant === o)
			)
				return;
		}
		const u = { mousedown: !0, mouseup: !1, click: !1, touchstart: !0, touchend: !1 },
			...
			m = new Map(),
			p = (e, t) => {
				const n = t instanceof HTMLInputElement ? t : null,
					o = (t) => {
						(v(t), w.add(e));
					};
				m.set(e, () => {
					for (const [e, r] of Object.entries(n ? u : f)) t.removeEventListener(e, r ? o : v, !0);
				});
				for (const [e, r] of Object.entries(n ? u : f)) t.addEventListener(e, r ? o : v, !0);
			};
		let h = !1;
		...
		window.raventicDropDownReady = function (e) {
			if (h) return;
			for (const [, e] of m) e();
			h = !0;
			const t = e(g, n, o, r, a, c, i, s, d);
		...
		((S.defer = !0), (S.src = "https://sdk.rvndn.com/semsearch/v4/dropdown.min.js"), document.head.appendChild(S));
	})();
}
...
// The SDK renders no close/clear controls on our custom-styled panel, so add our own
// .close-raventic and .clear-raventic buttons into the dropdown body. Runs on the next
// frame because the body may not be in the DOM yet at on-open time; idempotent so
// re-opens don't stack buttons.
function addDropdownControls() {
	requestAnimationFrame(() => {
		const dropdownBody = document.querySelector(".raventic-search-dropdown-body");
		if (!dropdownBody) return;

		if (!dropdownBody.querySelector(".close-raventic")) {
			const closeButton = document.createElement("div");
			closeButton.className = "close-raventic";
			closeButton.setAttribute("role", "button");
			closeButton.setAttribute("tabindex", "0");
			closeButton.setAttribute("aria-label", window.NAT.translations("raventicCloseLabel"));
			closeButton.addEventListener("click", closeRaventicDropdown);
			dropdownBody.appendChild(closeButton);
		}

		const searchFormFieldset = document.querySelector("#formSearchForm fieldset");

		if (!searchFormFieldset.querySelector(".clear-raventic")) {
			const clearButton = document.createElement("div");
			clearButton.className = "clear-raventic";
			clearButton.setAttribute("role", "button");
			clearButton.setAttribute("tabindex", "0");
			clearButton.setAttribute("aria-label", window.NAT.translations("raventicClearLabel"));
			clearButton.addEventListener("click", clearRaventicSearch);
			searchFormFieldset.appendChild(clearButton);
		}
	});
}

// The native search "×" can't clear the field (the vendor loader preventDefault()s
// pointer events on the input), so clear it ourselves and fire an input event so the
// widget re-queries. Selector comes from the config to stay in sync with the widget.
function clearRaventicSearch() {
	const selectors = window.RaventicSearchDropdownConfig?.elementSelectors;
	const selector = Array.isArray(selectors) ? selectors[0] : selectors;
	const input = selector ? document.querySelector(selector) : null;
	if (!input) return;

	// The widget is React-based: assigning input.value directly bypasses React's internal
	// value tracker, so its onChange never fires and the results don't reset. Clear via the
	// native value setter, then dispatch the events so React (and any plain listeners) pick
	// up the empty query and re-render. change/keyup cover non-React or keystroke-debounced
	// listeners too.
...
// We can't reach the widget instance's close() (it lives in the vendor loader closure),
// so close the way a user would: a pointer event outside the panel (this widget is
// mousedown/touch driven) plus Escape as a fallback. Closing an already-closed widget
// is a no-op, so firing both is safe.
function closeRaventicDropdown() {
	if (window.NAT.media.isMobile) {
		const mobileSearchButton = document.querySelector("#header .search-form");
...
/*--------------------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------------------------*/
//RAVENTIC SETUP RESULTS
// 2_components/search_raventic_results_setup_new.js
// Vendor Raventic search-RESULTS integration (test eshop only), v4 SDK: defines
// window.RaventicSearchResultsConfig and runs Raventic's loader snippet, which
// injects the v4 results SDK. The minified loader IIFE is vendor code, left untouched.

raventicSetupResults();
function raventicSetupResults() {
	/* if (!document.body.classList.contains("is-test-eshop")) return; */
	if (!location.href.includes(window.NAT.raventic[window.NAT.lang].resultsUrlMatch)) return;

	window.RaventicSearchResultsConfig = {
		...
		config: {
			...
			title: window.NAT.translations("raventicResultsTitle"),
			noQueryTitle: window.NAT.translations("raventicNoQueryTitle"),

			categoriesTitle: window.NAT.translations("raventicCategoriesTitle"),
			manufacturersTitle: window.NAT.translations("raventicManufacturersTitle"),
			...
			individualParameters: [
				...
				{
					parameterName: "RATING_AVG",
					title: window.NAT.translations("raventicParamRating"),
				},
				{
					parameterName: "RATING_TOTAL",
					title: window.NAT.translations("raventicParamReviewCount"),
				},
				{
					parameterName: "IMGURL_ALTERNATIVE",
					title: window.NAT.translations("raventicParamAltImage"),
				},
				{
					parameterName: "STOCK_AMOUNT",
					title: window.NAT.translations("raventicParamStock"),
				},
			],
	...
	// Vendor Raventic v4 loader — reads window.RaventicSearchResultsConfig and injects
	// .../semsearch/v4/results.min.js. Left exactly as provided by Raventic.
	!(function () {
		const e = window.RaventicSearchResultsConfig;
		if (!e)
			return void console.error(
				"RaventicSearchResultsConfig is not defined. Please set window.RaventicSearchResultsConfig before loading this script.",
			);
		const {
			elementSelector: t,
			config: n,
		...
		if (s) {
			...
			variant = r === a || r === i ? r : Math.random() < 0.5 ? a : i;
			try {
				window.localStorage.setItem(n, variant);
			} catch {}
			if (
		...
		let d = !1;
		window.raventicResultsReady = function (e) {
			d || ((d = !0), e(t, n, a, i, r, o, c));
		};
		const l = document.createElement("script");
		((l.src = "https://sdk.rvndn.com/semsearch/v4/results.min.js"), document.head.appendChild(l));
	})();
}

function handleProductClick(product, instanceId, action) {
	if (action === "add-to-cart") {
		shoptet.cartShared.addToCart({ productCode: product.id });
	}
}
...
/*--------------------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------------------------*/
//RAVENTIC PAGE RESULTS
// 3_pages/search_raventic_results.js
// Raventic search results page (test eshop only): transforms Raventic's product
// list into Shoptet-style product cards (rating, stock, price, add-to-cart),
// re-transforming via a MutationObserver, and adds a go-to-top button, filter
// heading, per-identifier classes on the promo-content blocks, and header-menu
// icons on the promo category links.

searchRaventicResults();
function searchRaventicResults() {
	/* if (!document.body.classList.contains("is-test-eshop")) return; */
	...
	document.addEventListener("RAVENTIC SEARCH RESULTS TRANSFORMED", () => {
		document.body.classList.add("raventic-search-results-transformed");
		addGoToTopButton();
		addFilterHeading();
	...
	function editRaventicSearchResults() {
		...
		function buildProductCard(rvProduct) {
			...
			if (cartBtn) {
				parts.push(
					`<div class="p-tools"><button type="button" class="btn btn-cart add-to-cart-button raventic-add-to-cart-button" data-testid="buttonAddToCart" aria-label="${window.NAT.translations("doKosiku")} ${escapeHtml(name)}"><span>${window.NAT.translations("doKosiku")}</span></button></div>`,
				);
			}
	...
	function addGoToTopButton() {
		const previousBtn = document.querySelector(".goToTop");
		if (previousBtn) {
			return;
		}
		...
		const raventicHeading = document.querySelector(".raventic-search-results-container");
		if (!raventicHeading) {
			return;
		}
		...
		goToTop.innerHTML =
			...
			window.NAT.translations("nahoru") +
	...
	function addFilterHeading() {
		...
		selectedFiltersHeading.textContent = window.NAT.translations("selectedFilters");
		raventicSelectedFiltersWrapper.prepend(selectedFiltersHeading);
	}

	// The v4 SDK renders every promo block with the same generic class, so add a
	// per-identifier modifier (e.g. raventic-search-results-promo-content-type--categories)
	// to allow styling each block separately. The title->identifier map is read from the
	// widget config (window.RaventicSearchResultsConfig) so there's a single source of truth.
	function addPromoContentClasses() {
		const promoContent = window.RaventicSearchResultsConfig?.config?.promoContent;
		if (!Array.isArray(promoContent)) return;

		const titleToIdentifier = new Map(promoContent.map((item) => [item.title, item.identifier]));

		const blocks = document.querySelectorAll(".raventic-search-results-promo-content-type");
		blocks.forEach((block) => {
			const titleEl = block.querySelector(".raventic-search-results-promo-content-type-title");
			const identifier = titleEl ? titleToIdentifier.get(titleEl.textContent.trim()) : null;
